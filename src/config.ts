...
export function requireConfig() {
  if (!config.oidSeedHex) throw new Error("Missing OID_SEED_HEX in environment");
  if (!config.oidDid) throw new Error("Missing OID_DID in environment");
  if (!config.oidGs1PackageId) throw new Error("Missing OID_GS1_PACKAGE_ID in environment");
  if (!config.oidGs1RegistryId) throw new Error("Missing OID_GS1_REGISTRY_ID in environment");

  // No auto-discovery: these MUST be provided explicitly.
  if (!config.oidCreditPackageId) throw new Error("Missing OID_CREDIT_PACKAGE in environment");
  if (!config.oidCreditPolicyId) throw new Error("Missing OID_CREDIT_POLICY_ID in environment");
  if (!config.oidCreditTokenId) throw new Error("Missing OID_CREDIT_TOKEN_ID in environment");
  if (!config.oidControllerCapId) throw new Error("Missing OID_CONTROLLER_CAP_ID in environment");
}
