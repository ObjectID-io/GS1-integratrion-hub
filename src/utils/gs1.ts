...
export function gs1CheckDigit(base: string): number {
  ...
  for (let i = digits.length - 1; i >= 0; i--) {
    ...
    if (d < 0 || d > 9) throw new Error("Invalid digit");
    sum += d * weight;
    weight = weight === 3 ? 1 : 3;
  }
  const mod = sum % 10;
...
export function toGtin14(companyPrefix: string, itemReference: string): string {
